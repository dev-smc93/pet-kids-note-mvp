...
| **보호자**                    | 알림장 작성(생활기록 제외)/열람, 읽음 처리, **본인 알림장 수정/삭제** |
...
4. 알림장 리스트
5. 알림장 상세 (읽음 표시)
6. 알림장 작성 (관리자 + 보호자) / 수정 (관리자: 관리자 글만, 보호자: 본인 글만)
...
- [x] 재알림(재전송): 미열람 사용자에게 다시 알림 (푸시 알람 연동)
- [x] 수정/삭제: 관리자(관리자 글만), 보호자(본인 글만)
...
- [x] 알림장 등록 시: 관리자 작성→보호자에게, 보호자 작성→원장에게 푸시 알람
