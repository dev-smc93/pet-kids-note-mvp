...
export function ReportsClient({
  ...
  const { profile, user, setProfileFromBootstrap } = useAuth();
  ...
  const currentGroupIds = searchParams.get("groupIds")
    ? searchParams.get("groupIds")!.split(",").filter(Boolean)
    : searchParams.get("groupId")
      ? [searchParams.get("groupId")!]
      : [];
  const currentMineOnly = searchParams.get("mineOnly") === "true";
  ...
  useEffect(() => {
    ...
    const paramsChanged =
      currentGroupIds.join(",") !== groupIds.join(",") || currentMineOnly !== mineOnly;
    if (paramsChanged) {
      const params = new URLSearchParams();
      if (currentGroupIds.length > 0) params.set("groupIds", currentGroupIds.join(","));
      if (currentMineOnly) params.set("mineOnly", "true");
      const url = `/api/bootstrap${params.toString() ? `?${params.toString()}` : ""}`;
      fetch(url)
        .then((res) => (res.ok ? res.json() : { profile: null, reports: [] }))
        .then((data: { profile: Profile; reports: ReportItem[] }) => {
          if (data.profile) setProfileFromBootstrap(data.profile);
          setReports(data.reports ?? []);
          const months = getMonthsFromReports(data.reports ?? []);
          if (months.length > 0) {
  ...
  }, [user, router, groupIds, mineOnly, currentGroupIds, currentMineOnly, setProfileFromBootstrap]);
  ...
  useEffect(() => {
    const m = getMonthsFromReports(reports);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
      <header className="sticky top-0 z-10 flex items-center justify-between bg-red-500 px-4 py-3">
        ...
        <Link
          href={
            currentGroupIds.length > 0 || currentMineOnly
              ? `/reports/filter?${new URLSearchParams({
                  ...(currentGroupIds.length > 0 ? { groupIds: currentGroupIds.join(",") } : {}),
                  ...(currentMineOnly ? { mineOnly: "true" } : {}),
                }).toString()}`
              : "/reports/filter"
          }
        ...
        <div className="mx-auto flex max-w-md flex-1 flex-col overflow-hidden">
          ...
          {filteredReports.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center py-16">
              ...
              {displayProfile.role === "ADMIN" && !hasReportsInOtherMonths && (
                      ...
                      <Link href={`/reports/${r.id}`}>
                        <div
                          className={`flex gap-4 rounded-lg p-4 shadow-sm transition ${
                            (displayProfile.role === "GUARDIAN" && !r.isRead && !r.isGuardianPost) ||
                            (displayProfile.role === "ADMIN" && r.isReadByAdmin === false && r.authorUserId !== displayProfile?.userId)
                              ? "bg-amber-50 hover:bg-amber-100"
                              : "bg-white hover:bg-zinc-50"
                          } ${(displayProfile.role === "GUARDIAN" && !r.isRead && !r.isGuardianPost) || (displayProfile.role === "ADMIN" && r.isReadByAdmin === false && r.authorUserId !== displayProfile?.userId) ? "border-l-4 border-amber-500" : ""}`}
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-sm font-medium text-zinc-900">
                              {r.isGuardianPost
                                ? `${r.pet.name} · 보호자의 글`
                                : `${r.groupName ? `${r.groupName} · ` : ""}${r.pet.name}${displayProfile.role === "ADMIN" && r.guardianName ? ` | ${r.guardianName}` : ""}`}
                            </p>
                            <p className="mt-0.5 line-clamp-2 text-sm text-zinc-600">{r.content}</p>
                            <div className="mt-1.5 flex items-center gap-2">
                              {displayProfile.role === "GUARDIAN" && !r.isRead && !r.isGuardianPost && (
                                <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">새 알림</span>
                              )}
                              {displayProfile.role === "ADMIN" && r.isReadByAdmin === false && r.authorUserId !== displayProfile?.userId && (
                                <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs text-amber-800">새 알림</span>
                              )}
                              {displayProfile.role === "ADMIN" && r.isReadByGuardian === false && !r.isGuardianPost && (
      ...
      {(displayProfile.role === "ADMIN" || displayProfile.role === "GUARDIAN") && (
        <Link
          href="/reports/new"
