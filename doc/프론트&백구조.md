...
│   │   └── [id]/
│   │       ├── page.tsx    # 알림장 상세
│   │       └── edit/page.tsx  # 알림장 수정 (관리자: 관리자 글만, 보호자: 본인 글만)
...
│   ├── reports/unread-count/  # GET (미확인 알림장 개수)
│   ├── push/subscribe/     # POST (푸시 구독), PATCH (활성화), DELETE (해제)
│   ├── bootstrap/          # GET (홈 초기 데이터)
...
├── push/                   # push-subscription-toggle, push-subscription-sync
├── reports/                # report-form, daily-record-form (생활기록)
├── providers/
└── ui/                     # button, input, select, confirm-dialog

lib/
├── api/auth.ts             # getAuthUser, requireAdmin
├── api/fetch-reports.ts    # fetchReportsList
├── auth/
├── constants/              # images.ts (이미지 경로 상수)
├── push/                   # send-push, sync-push-on-login, unsubscribe-on-logout
├── server/                 # home-data, bootstrap
...
| `/reports/new`               | ADMIN, GUARDIAN | 알림장 작성 (원 선택 → 반려동물 선택 → 작성, 보호자는 생활기록 제외) |
| `/reports/[id]`              | 인증          | 알림장 상세 (읽음, 댓글)            |
| `/reports/[id]/edit`         | ADMIN, GUARDIAN(본인 글만) | 알림장 수정                         |
...
| POST   | `/api/reports`              | 알림장 작성 (관리자: 생활기록 포함, 보호자: 생활기록 제외) | 인증   |
| GET    | `/api/reports/[id]`         | 알림장 상세                                 | 인증   |
| PATCH  | `/api/reports/[id]`         | 알림장 수정                                 | ADMIN(관리자 글만), GUARDIAN(본인 글만) |
| DELETE | `/api/reports/[id]`         | 알림장 삭제                                 | ADMIN(관리자 글만), GUARDIAN(본인 글만) |
| GET    | `/api/reports/unread-count` | 미확인 알림장 개수                          | 인증   |
| POST   | `/api/reports/[id]/read`    | 읽음 처리                                   | GUARDIAN |
| POST   | `/api/reports/[id]/remind`  | 재알림 (푸시 알람 연동)                     | ADMIN  |
...
| POST   | `/api/push/subscribe`        | 푸시 구독 등록 (body: endpoint, keys)       | 인증   |
| PATCH  | `/api/push/subscribe`        | 푸시 구독 활성화 (?endpoint=, ?active=true) | 인증   |
| DELETE | `/api/push/subscribe`        | 푸시 구독 해제 (?endpoint=)                 | 인증   |
| GET    | `/api/bootstrap`             | 홈 초기 데이터 (프로필, 원 개수, 미확인 알림장 수) | 인증   |
| POST   | `/api/upload/pet-photo`     | 반려동물 사진 업로드 (FormData)             | 인증   |
| POST   | `/api/upload/report-photo`   | 알림장 사진 업로드 (FormData)               | 인증   |
...
| 항목 | 설명 |
|------|------|
| 접근 | 관리자 + 보호자 (관리자: 생활기록 포함, 보호자: 생활기록 제외) |
| 원 선택 | 헤더에 선택된 원 이름 표시 ("알림장 작성 - [원 이름]") |
| 폼 순서 | 대상 반려동물 → 내용 → 사진(1~10장) → 생활기록(관리자만) |
| 생활기록 | 고정 버튼(화면 하단), 클릭 시 오버레이 패널(Framer Motion 슬라이드업) |
| 내용 입력 | textarea rows 8 |

## 10. 알림장 상세 UI

| 항목 | 설명 |
|------|------|
| 레이아웃 | 제목(고정) → 스크롤 영역(생활기록 → 이미지 → 내용) |
| 이미지 | Swiper 슬라이더, 2개 이상 시 점 인디케이터, 클릭 비활성화 |
| 스크롤 | 스크롤바 숨김, 아래 내용 있을 때 바운스 애니메이션 화살표 표시 |
| 스크롤 화살표 | overflow 시에만 표시, 20px 이상 스크롤 시 숨김 |
| 수정/삭제 | 관리자: 관리자 글만, 보호자: 본인 글만 표시 |
| 재알림 | 관리자: 보호자 미읽음 시 "재알림" 버튼 표시 (푸시 알람 연동) |
...
| 애니메이션 | Framer Motion (생활기록 오버레이)           |
| 슬라이더  | Swiper (알림장 상세 이미지)                 |
...
| 항목 | 설명 |
|------|------|
| 설치형 웹앱 | manifest + 아이콘 + 서비스워커(기본 캐싱) |
| 푸시 알람 | 알림장 등록(관리자→보호자, 보호자→원장), 댓글 등록, 재알림 시 발송 |
| 재알림 푸시 | 재알림 버튼 클릭 시 미열람 보호자에게 푸시 알람 발송 (구현 완료) |
